...
export default function ArtistsEdit({ artist }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        bio: artist.bio || '',
        social_links: artist.social_links ? JSON.stringify(artist.social_links) : '',
        is_active: artist.is_active || true,
    });

    const [previewImage, setPreviewImage] = useState(artist.profile_image || '');
    ...
    const submit = (e) => {
        e.preventDefault();
        put(`/artist-panel/artists/${artist.id}`, {
    ...
    return (
        <AdminLayout title="Edit Artist">
            <Head title={`Edit ${artist.name} - Admin Panel`} />
            
            <div className="space-y-6 pb-20">
                {/* Header Section */}
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-k2d-bold text-gray-900">Edit Artist</h2>
                        <p className="mt-1 text-sm text-gray-600">
                            Update "{artist.name}" profile details
                        </p>
                    </div>
                    <Link
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    <h4 className="font-medium text-gray-900 mb-2">Artist Info</h4>
                                    <div className="space-y-1 text-sm text-gray-600">
                                        <p><span className="font-medium">ID:</span> #{artist.id}</p>
                                        <p><span className="font-medium">Created:</span> {new Date(artist.created_at).toLocaleDateString()}</p>
                                        <p><span className="font-medium">Last Updated:</span> {new Date(artist.updated_at).toLocaleDateString()}</p>
                                        {artist.products_count !== undefined && (
                                            <p><span className="font-medium">Products:</span> {artist.products_count}</p>
                                        )}
                                    </div>
                                </div>
